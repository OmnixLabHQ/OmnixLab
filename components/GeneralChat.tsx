...
export default function GeneralChat() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'messages', filter: 'project_id=is.null' },
        () => fetchMessages()
      )
      .subscribe()
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-bold text-gray-900">General Chat</h3>
          <p className="text-xs text-gray-500">Chat with our team</p>
        </div>
      </div>

      <div className="h-56 overflow-y-auto space-y-3 bg-gray-50 rounded-xl p-3">
        {messages.map(msg => (
