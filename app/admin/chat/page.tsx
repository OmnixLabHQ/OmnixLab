...
export default function AdminChatPage() {
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    if (filter === 'project') return conv.project_id !== null
    if (filter === 'support') return conv.category === 'support'
    if (filter === 'billing') return conv.category === 'billing'
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <input
          ...
          className="flex-1 px-4 py-2.5 bg-white/10 border border-white/20 text-white rounded-lg text-sm placeholder-gray-500"
        />
        <select
          value={priorityFilter}
          onChange={(e) => setPriorityFilter(e.target.value)}
          className="px-4 py-2.5 bg-white/10 border border-white/20 text-white rounded-lg text-sm"
        >
              ...
              <div className="p-4 border-t border-white/10">
                <div className="flex items-end gap-2">
                  ...
                  <textarea
                    ...
                    rows={1}
                    className="flex-1 px-4 py-2 bg-white/10 border border-white/20 text-white rounded-lg text-sm resize-none"
                  />
                  <button
