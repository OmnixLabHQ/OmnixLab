...
const STEPS = ['Project', 'Business', 'Scope', 'Budget', 'Files', 'Review']
...
export default function StartProjectPage() {
  ...
  const [form, setForm] = useState({
    ...
    existingSystemActions: [] as string[],
    features: [] as string[],
    otherFeatures: '',
  ...
  useEffect(() => {
    // Pre-fill company from client profile if available
    const fetchProfile = async () => {
      ...
      if (user) {
        const { data: client } = await supabase
          ...
          .single()
        if (client) {
          setForm(prev => ({ ...prev, company: client.company || '' }))
        }
      }
    }
    fetchProfile()
  ...
  const handleNext = () => {
    if (step === 0) {
      if (!form.projectName.trim() || !form.projectType || form.description.trim().length < 10) {
        setError('Please provide a project name, type, and a description of at least 10 characters.')
        return
      }
    }
    if (step === 1) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (form.features.length > 0) {
        const featureRows = form.features.map(feature => ({
          request_id: request.id,
          feature: feature,
        }))
        await supabase.from('project_request_features').insert(featureRows)
      }

      // Insert target users
      if (form.targetUsers.length > 0) {
        const userRows = form.targetUsers.map(user => ({
          request_id: request.id,
          target_user: user,
        }))
        await supabase.from('project_request_target_users').insert(userRows)
      }

      // Insert platforms
      if (form.platforms.length > 0) {
        const platformRows = form.platforms.map(platform => ({
          request_id: request.id,
          platform: platform,
        }))
        await supabase.from('project_request_platforms').insert(platformRows)
      }

      // Insert integrations
      if (form.integrations.length > 0) {
        const integrationRows = form.integrations.map(integration => ({
          request_id: request.id,
          integration: integration,
        }))
        await supabase.from('project_request_integrations').insert(integrationRows)
      }
      ...
      try {
        await supabase.from('notifications').insert({
          ...
          message: `New project request ${requestNumber} from ${form.company}`,
          read: false,
          created_at: new Date().toISOString(),
        })
      } catch (e) {}
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={(e) => updateForm('description', e.target.value)} rows={4} className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-900" placeholder="Describe what you want to build, the problem it should solve..." />
              ...
              <div>
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2">
                    <input type="radio" checked={form.existingSystem === 'no'} onChange={() => updateForm('existingSystem', 'no')} className="w-4 h-4" />
                    <span className="text-sm text-gray-700">No</span>
                  </label>
                  <label className="flex items-center gap-2">
                    <input type="radio" checked={form.existingSystem === 'yes'} onChange={() => updateForm('existingSystem', 'yes')} className="w-4 h-4" />
                    <span className="text-sm text-gray-700">Yes</span>
                  </label>
                </div>
                {form.existingSystem === 'yes' && (
                  <div className="mt-3">
                    <input type="text" value={form.existingSystemDetails} onChange={(e) => updateForm('existingSystemDetails', e.target.value)} className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-900" placeholder="Existing website or application" />
                  </div>
                )}
              </div>
            </div>
          ...
          {step === 5 && (
            <div className="space-y-4">
              ...
              <div className="mt-4 space-y-2">
                ...
                <label className="flex items-start gap-2">
                  <input type="checkbox" checked={form.declaration} onChange={(e) => updateForm('declaration', e.target.checked)} className="w-4 h-4 mt-0.5" />
                  <span className="text-sm text-gray-700">I understand that submitting this request does not automatically constitute project approval or acceptance.</span>
                </label>
              </div>
            </div>
          )}
        ...
        <div className="flex justify-between mt-6">
          {step > 0 ? (
            <button onClick={handleBack} className="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-xl">
              Back
            </button>
          ) : <div />}
          {step < STEPS.length - 1 ? (
            <button onClick={handleNext} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl">
              Continue
            </button>
          ) : (
            <button
